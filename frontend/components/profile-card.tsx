import { getSkillNameById } from '@/utils/utils';
      ...
      <div className="mb-5">
        ...
        {user.education?.map((edu: any, index: any) => (
          <div key={index} className="mb-5">
            ...
            <p>Score: {edu.score || '-'}</p>
            <ul className="list-disc ml-5">
              {edu.courses?.map((course: any, i: any) => <li key={i}>{course}</li>)}
            </ul>
          </div>
        ))}
      </div>
      ...
      <div>
        ...
        {user.projects?.map((project: any, index: any) => (
          <div key={index} className="mb-5">
            ...
            <ul className="list-disc ml-5">
              {project.highlights?.map((highlight: any, i: any) => <li key={i}>{highlight}</li>)}
            </ul>
          </div>
        ))}
      </div>
