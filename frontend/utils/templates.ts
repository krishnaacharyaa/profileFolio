...
const generateSocialLinksHTML = (links: SocialLinksData): string => {
  const iconMap: { [key: string]: string } = {
    ...
    linkedin:
      'https://user-images.githubusercontent.com/88904952/234979284-68c11d7f-1acc-4f0c-ac78-044e1037d7b0.png',
    stackoverflow:
    ...
    instagram:
      'https://user-images.githubusercontent.com/88904952/234981169-2dd1e58f-4b7e-468c-8213-034ba62156c3.png',
  };

  return Object.entries(links)
    .filter(([_, url]) => url)
    .map(
      ([platform, url]) => `<a href="${url}" target="blank">
...
${
  Object.values(socialLinks).some((link) => link)
    ? `<p align="left" style="display: flex; justify-content: start; gap: 10px;">${generateSocialLinksHTML(socialLinks)}</p>
    `
    : ''
}
